...
const Card: React.FC<CardProps> = ({
  ...
  const baseClasses = [
    ...
    isClickable
      ? 'cursor-pointer hover:shadow-md hover:border-design-primary/30'
      : '',
    className,
  ]
    .filter(Boolean)
  ...
  const validDOMProps = Object.keys(restProps).reduce(
    (acc, key) => {
  ...
  const renderSquareCard = () => (
    <div
      className={`${baseClasses} w-64 h-64 overflow-hidden`}
      onClick={onClick}
      {...validDOMProps}
    >
      {/* Image Section */}
