...
import { CompactColorPalette, useColorGroups } from './CompactColorPalette';
...
const componentDefinitions = {
  atoms: [
    {
      name: 'Brand',
      description:
        'Complete brand identity system with variants, states, and customization options',
    },
    {
      name: 'Typography',
      description: 'Text components with different variants and styles',
    },
    {
      name: 'Buttons',
      description: 'Interactive button components with different variants',
    },
  ...
  molecules: [
    {
      name: 'Service Cards',
      description: 'Cards displaying service information',
    },
    { name: 'Request Cards', description: 'Cards for request management' },
    {
      name: 'Form Fields',
      description: 'Complex form elements built from atoms',
    },
  ],
  organisms: [
...
const blockDefinitions = [
  ...
  {
    name: 'Stats Cards',
    description: 'Statistics dashboard with metrics cards',
  },
  {
    name: 'Activity Table',
    description: 'Recent activity table with user actions',
  },
];
...
export const ThemePreviewContent: React.FC<ThemePreviewContentProps> = ({
  className,
}) => {
  const [viewMode, setViewMode] = useState<'components' | 'blocks'>(
    'components',
  );
  ...
  const { currentTheme } = useCompanyTheme();

  // Generate color groups for the compact palette
  const colorGroups = currentTheme ? useColorGroups(currentTheme) : [];
  ...
  const filteredComponents = React.useMemo(() => {
    ...
    const filtered: typeof componentDefinitions = {
      atoms: [],
      molecules: [],
      organisms: [],
    };
    ...
    Object.entries(componentDefinitions).forEach(([category, components]) => {
      ...
      filtered[category as keyof typeof componentDefinitions] =
        matchingComponents;
    });
      ...
      {viewMode === 'components' && (
        <Tabs defaultValue="atoms" className="w-full">
          ...
          <TabsContent value="atoms" className="space-y-6">
            ...
            {filteredComponents.atoms.some((c) => c.name === 'Brand') && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Comprehensive brand system with logos, text, colors, and
                    interactive states
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                        ...
                        <div>
                          ...
                          <div className="bg-muted/30 rounded-lg p-4 flex items-center justify-center">
                            <Brand
                              ...
                              primaryTextColor={brandConfig?.primaryTextColor}
                              secondaryTextColor={
                                brandConfig?.secondaryTextColor
                              }
                              monochromeMode={
                                brandConfig?.monochromeMode || 'none'
                              }
                  ...
                  <div>
                    ...
                    <div className="bg-gradient-to-br from-primary/5 to-secondary/5 rounded-lg p-6 border-2 border-dashed border-primary/20">
                      ...
                      >
                        🎨 This preview updates automatically as you modify
                        brand settings in the Theme Editor
                      </Typography>
                    </div>
                  </div>
            ...
            {filteredComponents.atoms.some((c) => c.name === 'Typography') && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Text components with different variants and styles
                  </CardDescription>
                </CardHeader>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Theme color palette with comprehensive color groups
                </CardDescription>
              </CardHeader>
              <CardContent>
                <CompactColorPalette
            ...
            {filteredComponents.atoms.some((c) => c.name === 'Buttons') && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Interactive button components with different variants
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    <Typography variant="p" className="mb-3 font-medium">
                      Variants:
                    </Typography>
            ...
            {filteredComponents.atoms.some((c) => c.name === 'Icons') && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Icon library with categories and variants
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <Typography variant="caption" color="muted">
                      {Object.values(iconCategories).reduce(
                  ...
                  <Tabs defaultValue="all" className="w-full">
                    ...
                    <TabsContent value="all" className="mt-4">
                      <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 lg:grid-cols-12 gap-3">
                        {Object.entries(iconCategories).flatMap(
                          ([categoryKey, icons]) =>
                            icons.map((iconName) => (
                              <div
                                key={`${categoryKey}-${iconName}`}
                                className="flex flex-col items-center p-2 rounded-md hover:bg-muted transition-colors group text-foreground"
                              >
                                <Icon
                                  name={iconName}
                                  size="md"
          ...
          <TabsContent value="molecules" className="space-y-6">
            ...
            {filteredComponents.molecules.some(
              (c) => c.name === 'Service Cards' || c.name === 'Request Cards',
            ) && (
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">Cards</CardTitle>
                  <CardDescription>
                    Card components combining multiple atoms
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {filteredComponents.molecules.some(
              (c) => c.name === 'Form Fields',
            ) && (
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">Form Components</CardTitle>
                  <CardDescription>
                    Complex form elements built from atoms
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <TabsContent value="organisms" className="space-y-6">
            {filteredComponents.organisms.length === 0 ? (
              ...
              <Typography
                ...
                className="text-center py-8"
              >
                Organism examples would include complex components like
                navigation bars, data tables, forms, and dashboard widgets that
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-3">
                    <Button variant="outline" icon="refreshCwIcon">
                      Refresh
                    </Button>
