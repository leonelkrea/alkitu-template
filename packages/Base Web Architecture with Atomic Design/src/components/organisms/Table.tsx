...
export interface TableProps {
  ...
  onSort?: (column: string, direction: 'asc' | 'desc', multiSort?: Array<{column: string, direction: 'asc' | 'desc'}>) => void;
...
const Table: React.FC<TableProps> = ({
  ...
  const [sortColumn, setSortColumn] = useState<string>(sorting?.defaultSort?.column || '');
  const [sortDirection, setSortDirection] = useState<'asc' | 'desc'>(sorting?.defaultSort?.direction || 'asc');
  const [multiSort, setMultiSort] = useState<Array<{column: string, direction: 'asc' | 'desc'}>>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [activeFilters, setActiveFilters] = useState<Record<string, any>>({});
  const [expandedRows, setExpandedRows] = useState<string[]>([]);

  // Cell renderer functions
  const renderCell = (column: TableColumn, value: any, record: any, index: number): React.ReactNode => {
    if (!column.render) {
      return (
    ...
    switch (renderer.type) {
      case 'text':
        ...
        return (
          <Typography 
            ...
            size={textRenderer.size || 'sm'}
            weight={textRenderer.weight}
      ...
      case 'badge':
        ...
        const badgeConfig = badgeRenderer.mapping?.[value] || { variant: badgeRenderer.variant || 'neutral' };
        return (
          <Badge 
            variant={badgeConfig.variant as any} 
            size={badgeRenderer.size}
          >
      ...
      case 'chip':
        ...
        const chipConfig = chipRenderer.mapping?.[value] || { color: chipRenderer.color || 'neutral' };
        return (
          <Chip
            variant={chipRenderer.variant}
            color={chipConfig.color as any}
            size={chipRenderer.size}
            removable={chipRenderer.removable}
            onRemove={chipRenderer.onRemove ? () => chipRenderer.onRemove!(value, record) : undefined}
          >
        ...
        return (
          <div className="flex items-center space-x-2">
            <Avatar
              ...
              status={avatarRenderer.showStatus ? record[avatarRenderer.statusField || 'status'] : undefined}
            />
      ...
      case 'button':
        ...
        const isDisabled = typeof buttonRenderer.disabled === 'function' 
          ? buttonRenderer.disabled(record) 
          : buttonRenderer.disabled;
        const isLoading = typeof buttonRenderer.loading === 'function'
          ? buttonRenderer.loading(record)
          : buttonRenderer.loading;
        const tooltip = typeof buttonRenderer.tooltip === 'function'
      ...
      case 'link':
        ...
        const href = typeof linkRenderer.href === 'function' 
      ...
      case 'number':
        ...
        return (
          <Typography variant="p" size="sm">
            {numberRenderer.prefix || ''}{formattedNumber}{numberRenderer.suffix || ''}
          </Typography>
        );
        ...
        return (
          <div className="flex items-center space-x-2">
            ...
            `}>
              <div
                className={`
                  ...
                  ${progressRenderer.color === 'success' ? 'bg-success' :
                    progressRenderer.color === 'warning' ? 'bg-warning' :
                    progressRenderer.color === 'error' ? 'bg-error' :
                    'bg-primary'
                  }
                `}
            ...
            {progressRenderer.showValue && (
              <Typography variant="p" size="xs" color="muted" className="min-w-[3rem] text-right">
      ...
      {selectable && selectedKeys.length > 0 && (
        <div className="flex items-center justify-between p-4 bg-accent/30 border border-border rounded-lg">
          <Typography variant="p" size="sm" weight="medium">
            {selectedKeys.length} elemento{selectedKeys.length !== 1 ? 's' : ''} seleccionado{selectedKeys.length !== 1 ? 's' : ''}
          </Typography>
          <div className="flex items-center space-x-2">
            <IconButton
              icon="Download"
              ...
              {loading ? (
                ...
                data.map((record, index) => (
                  <React.Fragment key={index}>
                      ...
                      {expandable && (
                        <td className={getCellPadding()}>
                          {(!expandable.rowExpandable || expandable.rowExpandable(record)) && (
                            <IconButton
                              icon={expandedRows.includes(index.toString()) ? "ChevronDown" : "ChevronRight"}
                              iconOnly
                              size="sm"
                              variant="ghost"
                      ...
                      {(actions.length > 0 || rowActions) && (
                        <td className={getCellPadding()}>
                          <div className="flex items-center justify-end space-x-1">
                            ...
                            {rowActions && (
                              <>
                                {rowActions.items.slice(0, rowActions.maxVisible || 2).map((action) => {
                                  const isDisabled = typeof action.disabled === 'function' 
                                    ? action.disabled(record) 
                                    : action.disabled;
                                  const isHidden = typeof action.hidden === 'function'
                                ...
                                {rowActions.items.length > (rowActions.maxVisible || 2) && (
                                  <IconButton
                                    icon="MoreHorizontal"
                                    iconOnly
                                    size="sm"
                    ...
                    {rowActions && expandedRows.includes(index.toString()) && rowActions.items.length > (rowActions.maxVisible || 2) && (
                      <tr className="bg-accent/20">
                        <td colSpan={
                          columns.length + 
                          (selectable ? 1 : 0) + 
                          ...
                          <div className="flex items-center space-x-2">
                            <Typography variant="p" size="sm" weight="medium" className="mr-4">
                              Acciones adicionales:
                            </Typography>
                            {rowActions.items.slice(rowActions.maxVisible || 2).map((action) => {
                              const isDisabled = typeof action.disabled === 'function' 
                                ? action.disabled(record) 
                                : action.disabled;
                              const isHidden = typeof action.hidden === 'function'
