...
const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  const getEventColor = (type: string) => {
    switch (type) {
      case 'meeting': return 'bg-design-primary';
      case 'deadline': return 'bg-error';
      case 'maintenance': return 'bg-warning';
      case 'training': return 'bg-success';
                ...
                <div className="space-y-4">
                  ...
                  {dayEvents.length === 0 && (
                    <div className="text-center py-4">
                      <Icon name="Calendar" size="lg" color="muted" className="mx-auto mb-2" />
                      <Typography variant="p" size="sm" color="muted">
    ...
    <div className={`space-y-6 ${className}`} {...props}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <Typography variant="h4" weight="medium" className="min-w-[200px] text-center">
              {getViewTitle()}
            </Typography>
            <IconButton
              icon="ChevronRight"
      ...
      {(viewMode === 'month' || viewMode === 'week') && (
        <Card variant="vertical" title="">
          <div className="p-6">
            <Typography variant="h4" weight="medium" className="mb-4">
              {viewMode === 'month' ? 'Eventos de hoy' : 'Eventos de la semana seleccionada'}
            </Typography>
            
            {(() => {
              const relevantEvents = viewMode === 'month' 
                ? getEventsForDate(new Date())
                : viewData.flatMap(date => getEventsForDate(date));
