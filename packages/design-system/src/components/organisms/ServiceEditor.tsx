...
const ServiceEditor: React.FC<ServiceEditorProps> = ({
  service,
  availableCategories = ['Hogar', 'Limpieza', 'Mantenimiento', 'Jardín', 'Emergencias'],
  onSave,
  onDelete,
  onCancel,
  className = '',
  ...
  const addQuestion = (type: FormQuestion['type']) => {
    const newQuestion: FormQuestion = {
      ...
      type,
      title: type === 'text' ? 'Nueva pregunta de texto' : 
             type === 'photo' ? 'Nueva pregunta de fotos' : 
             'Selección múltiple',
      required: false,
  ...
  const addMultiselectOption = (questionId: string) => {
    ...
    updateQuestion(questionId, {
      config: {
        ...
        options: [
          ...(formData.questions.find(q => q.id === questionId)?.config?.options || []),
          newOption
        ]
      }
    });
  ...
  const updateMultiselectOption = (questionId: string, optionId: string, updates: { text?: string; image?: string }) => {
  ...
  const renderQuestion = (question: FormQuestion) => (
    <div key={question.id} className="bg-neutral-50 border border-border rounded-lg p-4 space-y-4">
      <div className="flex items-start justify-between">
        <div className="flex-1 space-y-3">
          <div className="flex items-center space-x-2">
            <Icon 
              name={question.type === 'text' ? 'Type' : question.type === 'photo' ? 'Camera' : 'CheckSquare'} 
              size="sm" 
              color="muted" 
            />
            <Typography variant="p" size="sm" weight="medium" className="capitalize">
              {question.type === 'text' ? 'Texto' : question.type === 'photo' ? 'Fotos' : 'Selección múltiple'}
            </Typography>
            <Chip 
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <FormField
                ...
                inputProps={{
                  ...
                  onChange: (e) => updateQuestion(question.id, {
                    config: { ...question.config, maxLength: parseInt(e.target.value) }
                  }),
                  placeholder: '500'
                }}
              />
          ...
          {question.type === 'multiselect' && (
            <div className="space-y-3">
              ...
              {question.config?.options?.map(option => (
                <div key={option.id} className="grid grid-cols-2 gap-3 items-end p-3 bg-card border border-border rounded-md">
                  <FormField
                    label="Texto de la opción"
                    inputProps={{
                      value: option.text,
                      onChange: (e) => updateMultiselectOption(question.id, option.id, { text: e.target.value }),
                  ...
                  <FormField
                    ...
                    inputProps={{
                      value: option.image || '',
                      onChange: (e) => updateMultiselectOption(question.id, option.id, { image: e.target.value }),
                      placeholder: 'https://...'
                    }}
                  />
                  <div className="col-span-2 flex items-center justify-between">
                    ...
                    <Button
                      ...
                      size="sm"
                      icon="Trash2"
                      onClick={() => removeMultiselectOption(question.id, option.id)}
        ...
        <div className="flex items-center space-x-2 ml-4">
          <IconButton
            ...
            size="sm"
            tooltip={question.required ? 'Marcar como opcional' : 'Marcar como obligatorio'}
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-card border border-border rounded-lg p-6 space-y-6">
            <div className="flex items-center space-x-2 pb-4 border-b border-border">
              ...
              <Typography variant="h3" weight="medium">Información básica</Typography>
            </div>
          ...
          <div className="bg-card border border-border rounded-lg p-6 space-y-6">
            <div className="flex items-center justify-between pb-4 border-b border-border">
              <div className="flex items-center space-x-2">
                ...
                <Typography variant="h3" weight="medium">Preguntas del formulario</Typography>
              </div>
              <div className="flex items-center space-x-2">
                <Button
                  variant="outline"
              ...
              {formData.questions.length === 0 && (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <Typography variant="p" size="sm">
                    No hay preguntas configuradas. Añade preguntas para personalizar el formulario.
                  </Typography>
                </div>
              )}
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card border border-border rounded-lg p-6 space-y-4">
            <div className="flex items-center space-x-2 pb-4 border-b border-border">
              ...
              <Typography variant="h4" weight="medium">Imagen del servicio</Typography>
            </div>
            ...
            <div className="space-y-4">
              {thumbnailPreview ? (
                ...
                >
                  <Icon name="Upload" size="lg" color="muted" className="mb-2" />
          ...
          <div className="bg-card border border-border rounded-lg p-6 space-y-4">
            <div className="flex items-center space-x-2 pb-4 border-b border-border">
              ...
              <Typography variant="h4" weight="medium">Categoría</Typography>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {categories.map(category => (
                  <Chip
                    key={category}
                    label={category}
                    variant={formData.category === category ? 'primary' : 'neutral'}
                    clickable={true}
