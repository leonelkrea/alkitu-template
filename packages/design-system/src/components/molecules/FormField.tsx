...
const FormField: React.FC<FormFieldProps> = ({
  ...
  const [selectedCountryCode, setSelectedCountryCode] = useState(phoneConfig?.countryCode || '+1'); // Cambiado a USA por defecto
  ...
  const validInputProps = Object.fromEntries(
    Object.entries(props).filter(([key]) => 
      !['label', 'description', 'error', 'required', 'type', 'options', 'phoneConfig'].includes(key)
    )
  );
              ...
              {options.map((option) => (
                <SelectItem key={option.value} value={option.value}>
                  <div className="flex items-center space-x-2">
                    {option.icon && <Icon name={option.icon as any} size="sm" />}
                ...
                <SelectValue>
                  <div className="flex items-center space-x-2">
                    <span className="text-sm">
                      {COUNTRY_CODES.find(c => c.value === selectedCountryCode)?.flag || '🌍'}
                    </span>
